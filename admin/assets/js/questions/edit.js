    ...
    if (error_form === false) {
        $.ajax({
            ...
            beforeSend: function() {
                ...
                thisInst.html("<i class='fa fa-spinner fa-spin '></i> Updating...");
            },
            success: function(data) {
                data = JSON.parse(data);
                thisInst.prop("disabled", true).html("Question Updated");
                if (data.status == "success") {
                    $(".update-question-message").parent().addClass("alert-success").removeClass("d-none alert-danger");
                    $(".update-question-message").html(data.message);
                    thisInst.prop("disabled", false).html("Update Question");
                } else {
                    $(".update-question-message").parent().addClass("alert-danger").removeClass("d-none alert-success");
                    $(".update-question-message").html(data.message);
                    thisInst.prop("disabled", false).html("Update Question");
                }
...
function checkAnswer(name, clss) {
    ...
    if (check_bos_res == 0) {
        $("." + clss).find(".form-check-input")
            .removeClass("is-valid")
            .addClass("is-invalid")
        $("." + clss).find(".error-options-answer").addClass('text-danger').html("Please select correct answer");
        return false;
    } else {
        $("." + clss).find(".form-check-input")
            .removeClass("is-invalid")
