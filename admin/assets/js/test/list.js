            ...
            html += `<tr class="render-count-${data[i].test_id}">
                        ...
                        <td>
                            <div class="form-switch">
                                <input class="form-check-input" data-bs-toggle="tooltip" data-bs-placement="top" title="change status" type="checkbox" id="flexSwitchCheckChecked${data[i].test_id}" data-change-status-test change-status-id="${data[i].test_id}"  ${ (data[i].status ==1 ) ? "checked" : ''}>
                            </div>
                        </td>
...
$(document).on("click", "[data-change-status-test]", function(e) {
    ...
    $.ajax({
        method: "POST",
        url: "/ShopifyQuiz/admin/snippet/adminAjaxCall.php",
        data: { action: "admin/change-status-test", id: $(this).attr("change-status-id") },
        beforeSend: function() {
            $(".card-body").addClass('disabled-div');
        },
        success: function(data) {
